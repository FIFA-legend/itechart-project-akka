...
import com.itechart.project.domain.country.{Country, CountryCode, CountryId, CountryName}
...
trait CountryRepository {
  ...
  def findByName(name:     CountryName):   Future[Option[Country]]
  def findByCode(code:     CountryCode):   Future[Option[Country]]
